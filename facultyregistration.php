<?php
session_start();
header('location:fregistration.php');
...
if(isset( $_POST['username']) && isset( $_POST['password']) && isset( $_POST['confirmpassword'])){
        ...
        if($pass!=$cpass)
        {
          header('location:password_error_faculty_registration.php');
        }
        else{
            $q=" select * from faculty where username='$name';";
            $result = mysqli_query($con,$q);
            $num = mysqli_num_rows($result);
            echo $num;
            if($num==1)
            {
              header('location:userid_error_faculty_registration.php');
            }
            else {
            // now we need to write our insert query
              $qy="insert into faculty (username,password) values('$name','$pass')";
